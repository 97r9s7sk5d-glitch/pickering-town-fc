      ...
      {clubSponsors.map((s) => (
        ...
        >
          <div className="flex h-20 w-36 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-white p-2 shadow-lg sm:w-44">
            {s.logo ? (
              <img src={s.logo} alt={`${s.name} logo`} width={580} height={166} className="max-h-full w-full object-contain" loading="lazy" />
            ) : (
              <span aria-hidden="true" className="display text-center text-2xl leading-none text-ink">
                {s.name}
              </span>
            )}
          </div>
          <div className="min-w-0">
            <p className="eyebrow !text-[11px] text-pike-bright">Club sponsor</p>
